...
prognozującego przeżywalność pacjenta.
...
Na wykresie przedstawiono rozkłady czasu przeżycia całkowitego w
...
Przeżywalność w podziale na wiek: w grupie 60+ nieomal nie ma żywych
pacjentek - nie wynika to jednak koniecznie z typu operacji czyli
terapii, a po prostu z diagnozy w późnym wieku. Z kolei w grupie 20-40
przeżywa zdecydowanie więcej kobiet niż w 40-60. Warto też zauważyć, że
w niemal każdym przypadku (wieku, terapii etc.) śmierć spowodowana
postępem nowotoworu następuje znacznie później niż zgon spowodowany
ewentualnymi innymi schorzeniami bądź powikłaniami - na przeżywalność
wpływają również współistniejące lub poprzedzające schorzenia.

Przeżywalność jest większa w grupie operacji oszczędzającej;
niekoniecznie sugeruje to jej wyższość nad mastektomią - prawdopodobnie
jest możliwa tylko u pacjentek o lepszych rokowaniach.

W grupie 20-40 najdłuższym przeżyciem cechuje się albo sama
radioterapia, albo brak jakiejkolwiek terapii. Warto jednak pamiętać, że
wybór bardziej agresywnej kombinacji terapii i operacji niekoniecznie
skutkuje gorszymi rokowaniami, tylko zdarza się w cięższych przypadkach,
które same w sobie już slabo rokują.
...
Dla pacjentów którzy zmarli w wyniku raka poniższy wykres przedstawia
rozkład prawdopodobieństwa lat przeżytych od diagnozy
...
przykład średnia ilość prognozowanych misięcy przeżycia zmniejsza się
...
Liczności poziomów zmiennej wynikowej są w miarę podobne, przynajmniej
akceptowalne i nie wymagają modyfikacji.
...
Przypisano cyfry: 1. *Claudin-low*
...
Wynik ten jest zgodny z biologicznym charakterem klasyfikacji PAM50.
Poszczególne podtypy raka piersi nie stanowią całkowicie odrębnych
jednostek, lecz reprezentują kontinuum zmian molekularnych. Obserwowane
grupowanie sugeruje, że komponenty ICA uchwyciły część sygnałów
odpowiadających za zróżnicowanie profili ekspresji genów pomiędzy
podtypami nowotworu.
...
Kolejnym modelem będzie *eXtreme Gradient Boosting*. W tym wypadku nie
