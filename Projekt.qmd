...
Tematem projektu jest analiza zbioru danych Breast Cancer Gene Expression Profiles (METABRIC) zawierającego informacje dotyczące cech klinicznych pacjentów oraz molekularnych oraz budowa modelu prognozującego przeżywalność pacjenta.

## Opis zbioru

Zbiór składa się z 30 zmiennych klinicznych 173 zmiennych binarnych informujące o braku lub mutacji danego genu i 489 zmiennych dotyczących poziomu ekspresji genów.
```{python}
import pandas as pd
...
opisy = [
    ...
    "Prognozowany czas przeżycia w miesiącach",
    "Status przeżycia",
...
Jest niemal pewne, że uznamy odrzucimy hipotezę zerową niesłusznie przynajmniej raz. Dlatego zastosowano poprawkę Benjaminiego-Hochberga.
...
Na wykresie przedstawiono rozkłady czasu przeżycia całkowitego w miesiącach dla różnych kombinacji terapii, z podziałem na grupy wiekowe oraz rodzaj przeprowadzonego zabiegu chirurgicznego.
![Czas przeżycia w zależności od terapii, wieku i rodzaju operacji](fig.png)

*Do uzupełnienia dla Lidii*
...
Dla pacjentów którzy zmarli w wyniku raka poniższy wykres przedstawia rozkład prawdopodobieństwa lat przeżytych od diagnozy
...
display(p)
```
Liczności poziomów zmiennej wynikowej są w miarę podobne, przynajmniej akceptowalne i nie wymagają modyfikacji.
...
Jako, że rozmiar guza potrafi przyjmować bardzo wysokie wartości, postanowiono zbadać obecność obserwacji odstających. Przyjęto wariant wielowymiarowy aby uwzględnić relacje pomiędzy zmiennymi.
Wykorzystano do tego odległość mahalanobisa.
...
Analiza niezależnych składowych, teoretycznie wydaje się lepszym wyborem. Założono, że zmienne dotyczący ekspresji genów można opisać za pomocą mniejszej liczby komponentów reprezentujących niezależne procesy biologiczne.
Niech:
...
$S$ – macierz składowych niezależnych,
$A$ – macierz wag (mixing matrix).
...
$X_{ij}$ oznacza rzeczywistą wartość zmiennej,
$\hat{X}_{ij}$ oznacza wartość odtworzoną na podstawie modelu ICA.
...
```
Dominuje prawoskośność przy zróżnicowanym współczynniku kurtozy. Właściwości te są korzystne z punktu widzenia metody ICA, która opiera się na identyfikacji statystycznie niezależnych i niegaussowskich sygnałów ukrytych w danych.
...
1. *Liu J., Wang C. i wsp. TGF-β in Tumor Development and Progression: Mechanisms and Therapeutics. Molecular Biomedicine, 2026.*

2. *Santolla M.F., Maggiolini M. The FGF/FGFR System in Breast Cancer: Oncogenic Features and Therapeutic Perspectives. Cancers, 2020.*
3. *Gopalakrishnan S., Patel D., Shah M., Relan S. A Review of ARID1A's Role in Breast Cancer Progression. Cancers, 2026.*

4. *Huang Z., Zhou J. i wsp. RBPJ Maintains Brain Tumor-Initiating Cells Through CDK9-Mediated Transcriptional Elongation. Journal of Clinical Investigation, 2016.*

5. *Harrington W.R., Sengupta S. i wsp. Estrogen Regulation of the Glucuronidation Enzyme UGT2B15 in Estrogen Receptor-Positive Breast Cancer Cells. Endocrinology, 2006.*

6. *Jiang X., Xiang W., Wang X. i wsp. Tumor Suppressor FBXW7 and Its Regulation of DNA Damage Response and Repair. Frontiers in Cell and Developmental Biology, 2021.*
...
Wynik ten jest zgodny z biologicznym charakterem klasyfikacji PAM50. Poszczególne podtypy raka piersi nie stanowią całkowicie odrębnych jednostek, lecz reprezentują kontinuum zmian molekularnych. Obserwowane grupowanie sugeruje, że komponenty ICA uchwyciły część sygnałów odpowiadających za zróżnicowanie profili ekspresji genów pomiędzy podtypami nowotworu.
...
Model zbuodwano na poniższych parametrach:
...
```{python}
import pandas as pd
tabela=pd.read_csv('total_metryki_model1.csv', sep=';')
display(
    tabela.head(20).style
    .hide(axis='index')
    .set_table_styles([
...
```{python}
import pandas as pd
tabela=pd.read_csv('total_metryki2.csv', sep=';')
display(
    tabela.head(20).style
    .hide(axis='index')
    .set_table_styles([
...
Przebieg wartości metryk podczas treningu:

![](Metryki_over_drzewa_clin.png)
...
```{python}
import pandas as pd
tabela=pd.read_csv('total_metryki_clin.csv', sep=';')
display(
    tabela.head(20).style
    .hide(axis='index')
    .set_table_styles([
...
Model zbudowany tylko na zmiennych klinicznych jest nieznacznie lepszy.

## Random Forest

Konkurentem dla Cat boost jest Random Forest. Zastosowano porządkowe zakodowanie klas zmiennych kategorycznych, ponieważ OneHotEncoder zwiększył by wymiarowość. 
...
Random forest sprawdza się delikatnie gorzej niż Cat boost, winowajcą może być *ordinal encoding*
...
```
Ponownie model zbudowany tylko na klinicznych jest lepszy.
...
Kolejnym modelem będzie *eXtreme Gradient Boosting*. W tym wypadku nie tylko kategoryczne predyktory zostały zakodowane porządkowo ale również zmienna wyjściowa przyjmuje teraz wartości 0,1,2 odpowiednio dla *Died of Disease*, *Died of Other Causes*, *Living*.
...
Losowy model jest bardzo słaby pod względem metryk.
...
Co ciekawe wbrew oczekiwaniom dodanie komponentów ICA i mutacji genowych poprawia model.
